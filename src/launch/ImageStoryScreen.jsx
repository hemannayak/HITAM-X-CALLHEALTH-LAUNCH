...
const STORY_NODES = [
  ...
  { id: 'hospitals', label: 'Medicos', icon: '🏥', img: '/Phase2/Hospitals.png' },
...
export default function ImageStoryScreen({ advance }) {
  ...
  useEffect(() => {
    ...
    // Dismiss the intro instantly right as first card appears (5s)
    const introTimer = setTimeout(() => setShowIntro(false), 5000)

    // Start module cards after 5s (same moment intro dismisses)
    ...
    }, 5000) // 5s gap from phase start → first card (ABDM)
  ...
  return (
    <div className={styles.screen}>
      ...
      <div className={`${styles.introOverlay} ${!showIntro ? styles.introGone : ''}`}>
        ...
        <h2 className={styles.introHeading}>Built on <span className={styles.introAccent}>8 Core Integrations</span></h2>
